...
{
	return(glm::vec3(a * glm::vec4(b, 1.)));
}
...
ScalePointAroundAnotherPoint( glm::vec3 inputPoint, glm::vec3 centerPoint, glm::vec3 scale )
{
	...
	glm::mat4 scaledValue = glm::scale(glm::mat4(1. ), scale);
    glm::vec4 result = scaledValue * glm::vec4(relativePoint, 1.);
    glm::vec3 recentered = glm::vec3(result) + centerPoint;
    return(recentered);
...
{
    glm::vec3 relativePoint = inputPoint - centerPoint;
	glm::vec4 rotatedValue = third * second * first * glm::vec4(relativePoint, 1.);
    glm::vec3 absolutePoint = glm::vec3(rotatedValue) + centerPoint;
