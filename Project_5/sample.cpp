...
float	Dt;
struct particle *particle;		// array of structures	
GLUI *	Glui;			// instance of glui window
int	GluiWindow;		// the glut id for the glui window
int	LeftButton;		// either ROTATE or SCALE
int	MainWindow;		// window id for main graphics window
int	NumParticles;		// # fires		
int	ParticleList;
...
Animate( void )
{
	...
	{
		// dt = Time - particle[i].t0;
		particle[i].x = particle[i].x0 + particle[i].vx * Time;
		particle[i].y = particle[i].y0 + particle[i].vy * Time - 4.9 * Time * Time;
		particle[i].z = particle[i].z0 + particle[i].vz * Time;
	}
...
Display( void )
{
	...
	for(int i=0; i < NUMPARTICLES; i++)
	{
		// if( particle[i].t0 <= Time  &&  Time <= particle[i].t1 )
		{
...
InitGraphics( void )
{
	...
	particle = new struct particle [ NUMPARTICLES ];

	for(int i = 0; i < NUMPARTICLES; i++)
	{
		particle[i].x0 = Ranf(-1., 1.);
		particle[i].y0 = Ranf(-1., 1.);
		particle[i].z0 = Ranf(-1., 1.);
		float v = 0.;
		float th = 0.;
		particle[i].vx = Ranf(-1., 1.);
		particle[i].vy = Ranf(-1., 1.);
		particle[i].vz = Ranf(-1., 1.);
		particle[i].t0 = 0.;
		particle[i].t1 = 0.;
		particle[i].rad = 0.;
		particle[i].r = Ranf(0., 1.);
		particle[i].g = Ranf(0., 1.);
		particle[i].b = Ranf(0., 1.);
		particle[i].ti = 0;
...
Reset( void )
{
	...
	int j;
	struct particle *p;
	for( i=0, p=particle; i < NUMPARTICLES; i++, p++ )
	{
		particle[i].x = particle[i].x0;
		particle[i].y = particle[i].y0;
		particle[i].z = particle[i].z0;
		if( DoTraces )
		{
			particle[i].ti = 0;
			for( j=0; j < NUMTRACES; j++ )
			{
				particle[i].tx[j] = particle[i].x;
				particle[i].ty[j] = particle[i].y;
				particle[i].tz[j] = particle[i].z;
			}
		}
	}
